...
class OrderOutputService extends OutputService
{
    ...
    public static function getCollection(array $ids, $showDeleted = false, $imageSize = 'large'): array
    {
        ...
        return array_map(static function ($model) use ($imageSize, $userCurrency) {
            ...
            if ($info['buyerOffer']) {
                $info['price']['product_overall'] = RateService::convertValue($info['buyerOffer']['price_product'] * $info['buyerOffer']['total_quantity'], $info['buyerOffer']['currency'], $userCurrency);
            }
            unset(
                // $info['created_at'],
                // $info['status'],
